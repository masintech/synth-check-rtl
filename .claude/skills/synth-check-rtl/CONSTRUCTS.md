...
|-------------|----------|------------------------------------------------------|-------------------------------------------------|------------------------------------------------------------------------------------|
| `#delay`    | error    | No timing; synthesis ignores or drops it.            | Some emulators accept `#delay` in a sim-only region if isolated and pruned. | `always_ff @(posedge clk or negedge rst_n) if (!rst_n) q<=0; else q<=1;` — drive from the clock edge, not a delay. <!-- name:#delay | pattern:^(?!.*\bassign\b.*#\d+).*#\d+ -->
...
| `real`/`time` type | error | Non-synthesizable types.                       | Unsupported.                                    | Use fixed-point integer arithmetic. <!-- name:real/time | pattern:(?<!parameter\s)\b(real|realtime|time)\b -->
...
| associative array    | error    | Hashed storage; no hardware.                 | Unsupported.                       | Use a fixed-size array indexed by an enumerated/bounded key. <!-- name:associative array | pattern:\[\s*(\*|string|int|integer)\s*\] -->
...
| mixed logic in sequential block        | note     | A single-operator expression (e.g. `count <= count + 1`) synthesizes correctly on the flop's D-input; folding a larger combinational cloud into the sequential block still works but hurts readability and timing-closure separation. | N/A — a style/refactor suggestion, not a functional defect. | Move combinational logic to an `always @*` block; keep the sequential block a clean register (see SYNTH-RULES Rule 3). <!-- name:mixed logic in sequential block | semantic:mixed-logic -->
...
| `real`/`time` parameter            | error    | Non-synthesizable param type.                    | Unsupported.           | Use integer parameters. <!-- name:real parameter | pattern:parameter\s+(real|realtime|time)\b -->
