...
|-------------|----------|------------------------------------------------------|-------------------------------------------------|------------------------------------------------------------------------------------|
| `#delay`    | error    | No timing; synthesis ignores or drops it.            | Some emulators accept `#delay` in a sim-only region if isolated and pruned. | Remove; drive transitions from a clock edge. For a fixed wait, use a counter. |
...
| `wait(sig)` | error    | Level-sensitive wait has no hardware.                | Few emulators support it; usually rejected.     | Replace with edge detection: `@(posedge clk)` and a state flag.                    |
